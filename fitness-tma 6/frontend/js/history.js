...
const History = {
  ...
  renderDetailEntries() {
    ...
    container.innerHTML = this.editingWorkout.entries.map((entry, ei) => {
      const isCardio = isCardioSets(entry.sets);
      const setsHtml = entry.sets.map((s, si) => isCardio ? `
            <div class="flex gap-2 items-center text-sm">
              ...
              <input type="number" value="${s.speed ?? ''}" placeholder="км/ч" oninput="History.updateSet(${ei},${si},'speed',this.value)" class="input-field flex-1 !py-1.5">
              <span class="text-tg-hint text-xs">·</span>
              <input type="number" value="${s.duration ?? ''}" placeholder="мин" oninput="History.updateSet(${ei},${si},'duration',this.value)" class="input-field flex-1 !py-1.5">
            </div>` : `
            <div class="flex gap-2 items-center text-sm">
              <span class="w-5 h-5 rounded-full bg-accent/15 text-accent text-[10px] font-bold flex items-center justify-center shrink-0">${si + 1}</span>
