...
const Shop = {
  ...
  async load() {
    // Показываем состояние загрузки сразу — иначе на медленной сети/при
    // "холодном старте" Render экран будет выглядеть пустым и непонятным
    document.getElementById("shop-list").innerHTML = `
      <div class="text-center py-10 text-tg-hint">
        <i data-lucide="loader-2" class="w-8 h-8 mx-auto mb-2 animate-spin opacity-60"></i>
        <p class="text-sm">Загружаем тренировки...</p>
      </div>`;
    ...
    try {
      ...
      document.getElementById("shop-list").innerHTML = `
        <div class="text-center py-10 text-tg-hint">
          <i data-lucide="wifi-off" class="w-8 h-8 mx-auto mb-2 opacity-50"></i>
          <p class="text-sm mb-3">Не удалось загрузить тренировки.<br>Проверь соединение и попробуй ещё раз.</p>
          <button onclick="Shop.load()" class="btn-chip !w-auto px-5 inline-flex"><i data-lucide="refresh-cw" class="w-4 h-4"></i> Повторить</button>
        </div>`;
    }
    window.lucide?.createIcons();
  },
