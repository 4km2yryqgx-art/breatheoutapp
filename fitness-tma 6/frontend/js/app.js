...
const Home = {
  render() {
    ...
    API.get("/api/workouts/today-count").then((info) => {
      const btn = document.getElementById("start-workout-btn");
      if (!btn) return;
      if (info.reached) {
        btn.disabled = true;
        btn.classList.add("opacity-50");
        btn.innerHTML = `<i data-lucide="lock" class="w-5 h-5"></i> Лимит 2 тренировки в день исчерпан`;
      } else {
        btn.disabled = false;
        btn.classList.remove("opacity-50");
        btn.innerHTML = `<i data-lucide="play" class="w-5 h-5"></i> Начать тренировку${info.count > 0 ? ` (${info.count}/2 сегодня)` : ""}`;
      }
      window.lucide?.createIcons();
    }).catch(() => {});
