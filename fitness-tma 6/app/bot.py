...
async def process_successful_payment(message: Message):
    """После оплаты Stars — либо активируем Premium, либо засчитываем покупку тренировки из магазина."""
    payload = message.successful_payment.invoice_payload or ""

    if payload.startswith("shop_"):
        await _handle_shop_purchase(message, payload)
    else:
        await _handle_premium_purchase(message)


async def _handle_premium_purchase(message: Message):
...
async def _handle_shop_purchase(message: Message, payload: str):
    """Покупка готовой тренировки из магазина. payload: shop_{telegram_id}_{template_id}"""
    from app.database import PurchasedWorkoutTemplate
    from app.shop_data import WORKOUT_TEMPLATES
        ...
        if user and template_id:
            existing = await session.execute(
                select(PurchasedWorkoutTemplate).where(
                    PurchasedWorkoutTemplate.user_id == user.id,
                    PurchasedWorkoutTemplate.template_id == template_id,
                )
            )
            if not existing.scalar_one_or_none():
                session.add(PurchasedWorkoutTemplate(user_id=user.id, template_id=template_id))
                await session.commit()

    title = template["title"] if template else "тренировка"
    await message.answer(
        f"✅ Оплата получена! Тренировка «{title}» теперь доступна в разделе «Магазин» — "
        f"можешь запускать её в любое время.",
    )
