...
async def ai_generate_workout(
    ...
    """Генерирует тренировку под самочувствие и доступное оборудование прямо сейчас."""
    result = await session.execute(select(Exercise))
    all_exercises = [
        {"id": e.id, "name": e.name, "muscle_group": e.muscle_group, "equipment": e.equipment}
        for e in result.scalars().all()
    ]
    workout = generate_ai_workout(all_exercises, data.location, data.feeling, data.muscle_focus)
