  ...
  renderDaysList() {
    ...
    list.innerHTML = this.trainingDays.map((d) => `
      <div class="glass-card-flat rounded-2xl p-3 flex justify-between items-center">
        <button class="flex items-center gap-2 flex-1 min-w-0 text-left active-press" onclick="Workouts.startFromDay(${d.id})">
          <div class="w-8 h-8 rounded-lg bg-accent/15 flex items-center justify-center shrink-0">
            <i data-lucide="play" class="w-3.5 h-3.5 text-accent"></i>
          </div>
          <span class="text-sm font-medium truncate">${d.name}</span>
        </button>
        <button onclick="Plans.deleteDay(${d.id})" class="w-7 h-7 rounded-full bg-white/5 flex items-center justify-center active-press shrink-0 ml-2">
